...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        <div
          className="relative bg-zinc-900 border border-white/10 rounded-xl max-w-3xl w-full max-h-[85vh] overflow-hidden shadow-2xl flex flex-col"
        ...
        >
          <button
            onClick={() => setSelectedImage(null)}
            className="absolute top-3 right-3 z-50 p-2 bg-black/60 hover:bg-white hover:text-black rounded-full text-white transition-all cursor-pointer border border-white/10 backdrop-blur-md"
          >
            <X size={20} />
          </button>
          ...
          {(selectedImage.title || selectedImage.description) && (
            <div className="p-6 bg-zinc-900 border-t border-white/5 shrink-0 overflow-y-auto max-h-[30vh]">
              {selectedImage.title && (
                <h3 className="text-xl font-bold mb-2 text-white">{selectedImage.title}</h3>
              )}
              {selectedImage.description && (
                <p className="text-zinc-400 text-sm leading-relaxed">{selectedImage.description}</p>
              )}
            </div>
          )}
        </div>
  ...
  if (!isMounted) {
    return (
      <div
        className="bg-white/5 rounded-lg animate-pulse flex items-center justify-center border border-white/10"
        style={{ width: containerSize, height: containerSize }}
      >
        <div className="text-white/20 font-mono text-sm">Initializing 3D Space...</div>
      </div>
    );
  }
  ...
  if (!images.length) {
    return (
      <div
        className="bg-white/5 rounded-lg border-2 border-dashed border-white/10 flex items-center justify-center"
        style={{ width: containerSize, height: containerSize }}
      >
        <div className="text-white/40 text-center font-mono">
